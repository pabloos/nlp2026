...
def leaderboard_json(
    ...
    return [LeaderboardEntry(team=r.team, f1=r.f1, precision=r.precision, timestamp=r.timestamp) for r in records]
...
def leaderboard_html(
    ...
    for i, r in enumerate(records, 1):
        ...
        <tr>
          ...
          <td class="precision">{r.precision:.4f}</td>
          <td class="ts">{ts}</td>
        </tr>"""

    now = datetime.now(timezone.utc).strftime("%H:%M:%S UTC")
    empty_row = '<tr><td colspan="5" class="empty">Sin submissions todavía — ¡sé el primero!</td></tr>'
...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    table {{ width: 100%; max-width: 780px; border-collapse: collapse; }}
    th {{
...
<body>
  <h1>NLP Lab 1 · Sentiment Analysis</h1>
  <p class="subtitle">IMDB movie reviews &nbsp;·&nbsp; F1-score (pos) &nbsp;·&nbsp; techo TF-IDF ≈ 0.90</p>
  <table>
    <thead>
      <tr><th>Rank</th><th>Alumno</th><th>F1 (pos)</th><th>Precision</th><th>Último submit</th></tr>
    </thead>
    <tbody>
